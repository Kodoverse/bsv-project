      ...
      <div class="flex items-center justify-between h-16">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-dropdown">
          <ul class="flex items-center space-x-2">
            ...
            <li>
              <router-link :to="{ name: 'business-detail' }"
                class="px-2 py-2 font-medium text-gray-300 transition-all duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">
                Shop
              </router-link>
            </li>
            ...
            <li v-else class="relative">
              <div class="flex items-center space-x-2">
                ...
                <div v-if="store.CurrentUser?.info?.profile_img" id="userImg" class="overflow-hidden">
                  <img class="object-cover w-8 h-8 rounded-full" :src="store.CurrentUser.info.profile_img"
                    :alt="store.CurrentUser.display_name" />
                </div>
                <div v-else id="userInitials"
        ...
        <div class="md:hidden" id="navbar-dropdown">
          <div class="px-2 pt-2 pb-3 mt-2 space-y-1 rounded-lg sm:px-3 bg-gray-800/95 backdrop-blur-sm">
            <router-link :to="{ name: 'home' }"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">Home</router-link>
            <router-link :to="{ name: 'aboutus' }"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">Chi
              siamo</router-link>
            <a href="#"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">Eventi</a>
            <router-link :to="{ name: 'articles' }"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">News</router-link>
            <router-link :to="{ name: 'shopping' }"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">Shop</router-link>
            <a href="#"
              class="block px-3 py-2 text-gray-300 transition-colors duration-200 rounded-lg hover:text-white hover:bg-gray-700/50">Contatti</a>
            <div v-if="!store.isLoggedIn" class="pt-2">
              <router-link :to="{ name: 'login' }"
                class="block px-3 py-2 font-medium text-center text-white transition-all duration-200 rounded-lg bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600">Login</router-link>
            </div>
          </div>
        </div>
      </div>
