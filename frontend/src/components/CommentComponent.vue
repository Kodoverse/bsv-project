      ...
      <article class="p-6 text-base bg-white rounded-lg dark:bg-gray-900" v-for="comment in comments" :key="comment.id">
        <footer class="flex items-center justify-between mb-2">
          <div class="flex items-center">
            <div
              class="rounded-full overflow-hidden inline-flex items-center mr-2 text-sm font-semibold text-gray-900 dark:text-white">
              <div v-if="comment.user.info.profile_img">
                <img class="w-6 h-6 " :src="comment.user.info.profile_img" :alt="comment.user.display_name" />
              </div>
              <div v-else class="w-6 h-6 flex items-center justify-center bg-gray-500">
                {{ comment.user.initials }}
              </div>
            </div>
            <div class="font-semibold text-gray-900 dark:text-white mr-3">
        ...
        <p v-if="editId !== comment.id" class="text-gray-500 dark:text-gray-400">
          {{ comment.comment }}
        </p>
        <div v-else>
