<template>
  <div class="min-h-screen text-white bg-gray-900">
    <!-- Header -->
    <div
      class="border-b border-orange-700 bg-gradient-to-r from-orange-900 via-orange-800 to-orange-900"
    >
      <div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
          <div>
            <h1
              class="text-3xl font-bold text-transparent bg-gradient-to-r from-orange-400 to-yellow-400 bg-clip-text"
            >
              {{
                currentBusiness
          ...
          <div v-if="store.isLoggedIn" class="text-right">
            <p class="text-sm text-orange-300">Your Points</p>
        ...
        <div class="flex flex-wrap justify-center gap-4 mt-6">
          <div v-if="currentBusiness">
            <button
              ...
              class="inline-flex items-center px-6 py-3 text-white transition-colors rounded-lg bg-gray-800/50 hover:bg-gray-700/50"
            >
          ...
          <div
            v-if="store.isLoggedIn && !showPurchaseHistory && !currentBusiness"
          >
            <button
              @click="showPurchaseHistory = true"
              class="inline-flex items-center px-6 py-3 text-white transition-colors rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700"
            >
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div v-if="showPurchaseHistory">
        ...
        <!-- Purchase Filters -->
        <div class="p-4 mb-8 bg-gray-800 rounded-xl">
          <div class="flex flex-wrap items-center gap-4">
            <div>
              <select
                v-model="purchaseFilters.status"
                @change="fetchPurchaseHistory"
                class="px-4 py-2 text-white bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500"
              >
        ...
        <div v-if="purchaseHistoryLoading" class="flex justify-center py-12">
          <div
            class="w-12 h-12 border-b-2 border-orange-500 rounded-full animate-spin"
          ></div>
        </div>

        <div v-else-if="purchaseHistory.length === 0" class="py-12 text-center">
          <svg
        ...
        <div v-else class="space-y-6">
          <div
            ...
            :key="purchase.id"
            class="p-6 bg-gray-800 border border-gray-700 rounded-xl"
          >
            <!-- Purchase Header -->
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center space-x-4">
                <!-- Product Image -->
                <div
                  class="flex-shrink-0 w-16 h-16 overflow-hidden bg-gray-700 rounded-lg"
                >
                  <img
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-400">
                    {{ purchase.partner?.name || "Unknown Business" }}
                  </p>
                  <div
                    class="flex items-center mt-1 space-x-4 text-sm text-gray-500"
                  >
            ...
            <div
              class="p-4 mb-4 border bg-gradient-to-r from-orange-900/20 to-yellow-900/20 border-orange-500/30 rounded-xl"
            >
              <div>
                <p class="mb-1 text-sm font-medium text-orange-300">
                  Redemption Code
                </p>
                <div class="flex items-center space-x-3">
                  <code
                    class="px-4 py-2 font-mono text-2xl font-bold text-white border border-gray-600 rounded-lg bg-gray-900/50"
                  >
                    {{ purchase.redemption_code }}
                  </code>
                  <button
                    @click="copyToClipboard(purchase.redemption_code)"
                    class="p-2 text-white transition-colors bg-orange-600 rounded-lg hover:bg-orange-700"
              ...
              <div class="mt-3 text-sm text-orange-200">
                <div v-if="purchase.status === 'pending'">
                  💡 <strong>Next step:</strong> Show this code to
                  {{ purchase.partner?.name || "the business" }} to redeem your
                  order
                </div>
                ...
                <div v-else-if="purchase.status === 'completed'">
                  🎉 <strong>Order completed:</strong> Thank you for your
                  purchase!
                </div>
                <div v-else-if="purchase.status === 'cancelled'">
                  ❌ <strong>Order cancelled:</strong> Your points have been
                  refunded
                </div>
              </div>
            </div>
            ...
            <!-- Order Timeline -->
            <div class="pl-4 space-y-2 text-sm border-l-2 border-gray-600">
              <div class="flex items-center space-x-2 text-gray-400">
                ...
                <span
                  >Order placed: {{ formatDateTime(purchase.created_at) }}</span
                >
              </div>

              <div
                v-if="purchase.confirmed_at"
              ...
              >
                ...
                <span
                  >Order confirmed:
                  {{ formatDateTime(purchase.confirmed_at) }}</span
                >
              </div>

              <div
                v-if="purchase.completed_at"
                class="flex items-center space-x-2 text-gray-400"
              >
                <svg
                  class="w-4 h-4 text-green-400"
                ...
                <span
                  >Order completed:
                  {{ formatDateTime(purchase.completed_at) }}</span
                >
              </div>
            </div>
          </div>
      ...
      <div v-if="!currentBusiness && !showPurchaseHistory">
        ...
        <div class="p-4 mb-8 bg-gray-800 rounded-xl">
          <div class="flex flex-wrap items-center gap-4">
            <div class="flex-1 min-w-64">
              <input
                v-model="businessFilters.search"
                @input="debouncedBusinessSearch"
                type="text"
                placeholder="Search businesses..."
                class="w-full px-4 py-2 text-white bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              />
            </div>

            <div>
              <select
                v-model="businessFilters.category"
        ...
        <div v-else-if="businesses.length === 0" class="py-12 text-center">
          <svg
            class="w-16 h-16 mx-auto mb-4 text-gray-600"
            fill="currentColor"
            viewBox="0 0 20 20"
        ...
        >
          <div
            ...
            class="p-6 transition-all duration-200 bg-gray-800 border border-gray-700 cursor-pointer rounded-xl hover:border-orange-500 group"
          >
            <!-- Business Logo -->
            <div class="flex items-center mb-4 space-x-4">
              <div class="flex-shrink-0 w-16 h-16 overflow-hidden rounded-xl">
                <img
                  v-if="business.business_logo"
              ...
              <div class="flex-1">
                <h3
                  class="text-lg font-semibold text-white transition-colors group-hover:text-orange-400"
                >
                  {{ business.business_name }}
                </h3>
                <span
                  class="inline-block px-3 py-1 mt-1 text-sm font-medium text-orange-400 capitalize rounded-full bg-orange-600/20"
                >
            ...
            <p
              v-if="business.business_description"
              class="mb-4 text-sm text-gray-300 line-clamp-2"
            >
            ...
            <div class="mb-4 space-y-2 text-sm text-gray-400">
              <div
                v-if="business.business_address"
            ...
            >
              <span class="text-sm text-gray-400">
                ...
                {{ business.products_count === 1 ? "product" : "products" }}
              </span>
              <svg
                class="w-5 h-5 text-orange-400 transition-transform group-hover:translate-x-1"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
      ...
      <div v-else-if="currentBusiness && !showPurchaseHistory">
        ...
        <div class="p-6 mb-8 bg-gray-800 border border-gray-700 rounded-xl">
          <div class="flex items-start space-x-6">
            <!-- Business Logo -->
            <div class="flex-shrink-0 w-20 h-20 overflow-hidden rounded-xl">
              <img
                v-if="currentBusiness.business_logo"
            ...
            <div class="flex-1">
              <div class="flex items-center mb-2 space-x-3">
                ...
                <span
                  class="px-3 py-1 text-sm font-medium text-orange-400 capitalize rounded-full bg-orange-600/20"
                >
              ...
              <p
                v-if="currentBusiness.business_description"
              ...
              <div class="flex flex-wrap gap-4 text-sm text-gray-400">
                <div
                  v-if="currentBusiness.business_address"
                  class="flex items-center space-x-2"
                >
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path
                ...
                <div
                  v-if="currentBusiness.contact_phone"
                  class="flex items-center space-x-2"
                >
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path
                ...
                <div
                  v-if="currentBusiness.business_website"
                  class="flex items-center space-x-2"
                >
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path
                  ...
                  <a
                    :href="currentBusiness.business_website"
                    target="_blank"
        ...
        <!-- Product Filters -->
        <div
          class="flex flex-wrap items-center gap-4 p-4 mb-8 bg-gray-800 rounded-xl"
        >
          <div class="flex-1 min-w-64">
            <input
        ...
        >
          <div
            ...
            :key="product.id"
            class="overflow-hidden transition-all duration-200 bg-gray-800 border border-gray-700 rounded-xl hover:border-orange-500"
          >
            <!-- Product Image -->
            ...
            <div class="p-4">
              <div class="flex items-start justify-between mb-2">
                ...
                <span class="ml-2 text-lg font-bold text-orange-400"
                  >{{ product.points_price }} pts</span
                >
              </div>
              ...
              <div class="flex items-center justify-between mb-4">
                <span
                  class="px-2 py-1 text-xs text-gray-500 capitalize bg-gray-700 rounded-full"
                >
                  {{ product.category }}
                </span>
                <span
                  v-if="product.cash_equivalent"
              ...
              <div class="flex items-center space-x-2">
                <div class="flex items-center flex-1 space-x-1">
                  <button
                    ...
                    "
                    class="w-8 h-8 text-white transition-colors bg-gray-700 rounded-full disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-600"
                  >
                    -
                  </button>
                  <input
                    ...
                    "
                    class="w-16 text-sm text-center text-white bg-gray-700 border border-gray-600 rounded"
                  />
                  <button
                    @click="increaseQuantity(product.id)"
                    :disabled="
                      product.stock_quantity !== -1 &&
                      quantities[product.id] >= product.stock_quantity
                    "
                    class="w-8 h-8 text-white transition-colors bg-gray-700 rounded-full disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-600"
                  >
                    +
                ...
                <button
                  ...
                  :disabled="
                    !canPurchase(product) || purchasing.has(product.id)
                  "
                  class="px-4 py-2 text-sm font-medium text-white transition-colors bg-orange-600 rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div v-if="getTotalCost(product) > userPoints" class="mt-2">
                <p class="text-xs text-red-400">
                  ...
                  {{ getTotalCost(product) - userPoints }} more)
                </p>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async purchaseProduct(product) {
      ...
      try {
        ...
        alert(
          `Successfully purchased ${quantity}x ${product.name}! Redemption code: ${response.data.redemption_code}`,
        );
      } catch (error) {
    ...
    async fetchPurchaseHistory() {
      ...
      try {
        ...
        if (
          this.purchaseFilters.status &&
          this.purchaseFilters.status.trim() !== ""
        ) {
