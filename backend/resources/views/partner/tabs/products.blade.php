<div class="space-y-6">
    ...
    <div class="bg-gray-800 rounded-xl p-4 flex flex-wrap gap-4 items-center">
        <form action="{{ route('partner.products.index') }}" method="GET" class="flex flex-wrap gap-4 w-full">
            ...
            <button type="submit"
                class="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors">Filter</button>
        </form>
    </div>
                ...
                <div
                    ...
                    <div class="h-48 bg-gray-700 relative overflow-hidden">
                        ...
                        <div class="absolute top-3 left-3">
                            <span
                                class="px-3 py-1 rounded-full text-xs font-semibold {{ $product->is_available ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30' }}">
                        ...
                        <div class="absolute top-3 right-3">
                            <span
                                class="px-3 py-1 rounded-full text-xs font-semibold border {{ $product->stock_quantity > 0 ? 'border-green-500 text-green-400' : 'border-red-500 text-red-400' }}">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="" method="POST" class="flex-1">
                                @csrf
                                <button type="submit"
                                    class="w-full px-4 py-2 rounded-lg {{ $product->is_available ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-green-600 hover:bg-green-700 text-white' }}">
                                    {{ $product->is_available ? 'Disable' : 'Enable' }}
                                </button>
                            </form>

                            <form action="{{ route('partner.products.destroy', $product->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
